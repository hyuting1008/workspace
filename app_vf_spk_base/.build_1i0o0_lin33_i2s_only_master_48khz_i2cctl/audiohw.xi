...
# 13 "C:\\\\Users\\\\user\\\\workspace\\\\app_vf_spk_base\\\\.build_1i0o0_lin33_i2s_only_master_48khz_i2cctl\\\\vfspk_base_i2s_master.h" 3
...
on tile[1] : port p_rst_shared = on tile[1]: 0x40500;
on tile[1] : port p_i2c_4 = on tile[1]: 0x40400;
# 46 "../src/extensions/audiohw.xc"
void AudioHwInit(chanend ?c_codec)
{
    ...
    par {
        [[distribute]] i2c_master_single_port(i_i2c, 1, p_i2c_4, 100, 0, 1, 0xC);
        {
            ...
            {i_i2c[0].write_reg(0x62, 0x03, 0xF9);};


            {i_i2c[0].write_reg(0x62, 0xBB, 0xD0);};


            {i_i2c[0].write_reg(0x62, 0x12, 0x69);};


            {i_i2c[0].write_reg(0x62, 0x2C, 0x10);};


            {i_i2c[0].write_reg(0x62, 0x3C, 0x30);};



            {i_i2c[0].write_reg(0x62, 0x31, 0x00);};


            delay_milliseconds(1);


            {i_i2c[0].write_reg(0x62, 0x03, 0xF8);};
    ...
    unsigned samRes_DAC, unsigned samRes_ADC)
{
    ...
    par {
        [[distribute]] i2c_master_single_port(i_i2c, 1, p_i2c_4, 100, 0, 1, 0xC);
        {
            ...
            {i_i2c[0].write_reg(0x62, 0x03, 0xFD);};


            if ((samFreq % 11025) == 0)
            {

                {i_i2c[0].write_reg(0x62, 0x10, 0x6D);};
                {i_i2c[0].write_reg(0x62, 0x2D, 0x07);};
            }
            else if ((samFreq % 8000) == 0)
            {

                {i_i2c[0].write_reg(0x62, 0x10, 0x4D);};
                {i_i2c[0].write_reg(0x62, 0x2D, 0x05);};
            }
            else
            {
                                                                                        ;
            ...
            {i_i2c[0].write_reg(0x4A, 2, val);};


            val = 0b00001000;
            {i_i2c[0].write_reg(0x4A, 4, val);};


            {data = i_i2c[0].read_reg(0x4A, 3, i2c_res);};
            val = data | 0b00000001;
            {i_i2c[0].write_reg(0x4A, 3, val);};


            val = 0b11000000;
            {i_i2c[0].write_reg(0x4A, 8, val);};

            if (1)
            {

                val = 0b01000010;
                {i_i2c[0].write_reg(0x4A, 9, val);};


                val = 0b00000000;
                {i_i2c[0].write_reg(0x4A, 0x11, val);};
            }
            else
            {
