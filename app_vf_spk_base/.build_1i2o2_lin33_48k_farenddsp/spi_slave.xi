# 1 "C:/Users/user/workspace/lib_spi/src/spi_slave.xc"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 141 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "C:/Users/user/workspace/lib_spi/src/spi_slave.xc" 2
...
# 3 "C:/Users/user/workspace/lib_spi/src/spi_slave.xc" 2
...
int clz(unsigned x);
# 4 "C:/Users/user/workspace/lib_spi/src/spi_slave.xc" 2
...
# 5 "C:/Users/user/workspace/lib_spi/src/spi_slave.xc" 2
...
# 6 "C:/Users/user/workspace/lib_spi/src/spi_slave.xc" 2
...
# 8 "C:/Users/user/workspace/lib_spi/src/spi_slave.xc" 2
...
void spi_slave(client spi_slave_callback_if spi_i,
     ...
     __builtin_set_port_inv (ss, 0x600f);

     __builtin_stop_clock(clk);
     __builtin_set_clk_src(clk, sclk);
     ...
     ss when __builtin_pins_eq(!1) :> ss_val;
     while(1){
         select {
             case ss when __builtin_pins_ne(ss_val) :> ss_val:{

                 if(!isnull(miso))
                     __builtin_clear_buff(miso);

                 if(ss_val != 1){
                     unsigned remaining_bits = __builtin_endin(mosi);
                     uint32_t data;
                     mosi :> data;
                     if(remaining_bits){
                 ...
                 if(!isnull(miso)){
                     ...
                     if(transfer_type == SPI_TRANSFER_SIZE_8){
                         ...
                         if((mode == SPI_MODE_1) || (mode == SPI_MODE_2)){
                             asm volatile ("setclk res[%0], %1"::"r"(miso), "r"(0x6));
                             __builtin_partout(miso, 1, data);
                             asm volatile ("setclk res[%0], %1"::"r"(miso), "r"(clk));
                             data = data>>1;
                             __builtin_partout(miso, 7, data);
                         } else {
                             __builtin_partout(miso, 8, data);
                         }
                     } else {
                         data = __builtin_bitrev(data);
                         if((mode == SPI_MODE_1) || (mode == SPI_MODE_2)){
                             ...
                             asm volatile ("setclk res[%0], %1"::"r"(miso), "r"(clk));
                             data = data>>1;
                             __builtin_partout(miso, 31, data);
                         } else {
                             miso <: data;
                         }
                     }
                     ...
                     if(transfer_type == SPI_TRANSFER_SIZE_8){
                         buffer = (__builtin_bitrev(buffer)>>24);
                     } else {
                         buffer = __builtin_bitrev(buffer);
                     }
                 }
                 __builtin_clear_buff(mosi);
                 break;
             ...
             case mosi :> int i:{
                 if(transfer_type == SPI_TRANSFER_SIZE_8){
                     if(!isnull(miso)){
                         ...
                         __builtin_partout(miso, 8, buffer);
                         buffer = spi_i.master_requires_data();
                         buffer = (__builtin_bitrev(buffer)>>24);
                     }
                     spi_i.master_supplied_data(__builtin_bitrev(i)>>24, 8);
                     ...
                     spi_i.master_supplied_data(__builtin_bitrev(i), 32);
                 }
                 break;
             }
