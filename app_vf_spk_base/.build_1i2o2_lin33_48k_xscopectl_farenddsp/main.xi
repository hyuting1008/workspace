...
# 567 "C:/Users/user/workspace/module_usb_audio/main.xc"
    ...
    interface beclear_if i_beclear; chan chan_aec[4]; chan chan_erl; chan c_led; interface control i_control[(3)]; chan c_xscope; chan c_dsp;
    par
    {
        on tile[0].core[0]: BeclearBuff(i_audMan, i_beclear); on tile[0]: BeClear_SmartTV_AEC(chan_aec, chan_erl, i_beclear, i_control[0]); on tile[1]: { dsp_init_chanend(c_dsp); led_init_chanend(c_led); BeClear_SmartTV_BAP(chan_aec, chan_erl, i_control[1]); } xscope_host_data(c_xscope); on tile[(1)]: xscope_client(c_xscope, i_control); on tile[0].core[0]: led_controller(c_led); on tile[0]: far_end_dsp(c_dsp, i_control[(3)-1]);
